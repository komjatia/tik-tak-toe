...
const Board: FC<BoardProps> = ({
  ...
  const [cells, setCells] = useState<CellValue[]>(
    Array(numberOfCells).fill(undefined)
  );
  ...
  useEffect(() => {
    ...
    if (winningCondition === "x") {
      if (winArr.some((p, w) => p.player === player2 && p.win === "x")) {
        const arr = winArr.filter(
          (player) => player.player === player2 && player.win === "x"
        )[0];
        arr.num++;
        arr.cell =
          localCell.filter((x) => x === "x").length < arr.cell
            ? localCell.filter((x) => x === "x").length
            : arr.cell;
      } else {
        winArr.push({
          player: player2,
      ...
      if (winArr.some((p, w) => p.player === player1 && p.win === "o")) {
        const arr = winArr.filter(
          (player) => player.player === player1 && player.win === "o"
        )[0];
        arr.num++;
        arr.cell =
          localCell.filter((x) => x === "o").length < arr.cell
            ? localCell.filter((x) => x === "o").length
            : arr.cell;
      } else {
        winArr.push({
          player: player1,
  ...
  return (
    <StyledBoardContainer>
      <p>
        Következő:{" "}
        {current === "x" ? player2 : current === "o" ? player1 : null}
      </p>
      <StyledBoard>
        {localCell
          ? localCell.map((cell, i) => (
